...
def create_model(X_train, y_train, model_name):
    ...
    callbacks = [
        ...
        ModelCheckpoint(model_path_storage, verbose=1)
    ]
...
# method to downsample training data and labels 
def data_downsample(data,train_image_count,img_width,img_height):
    
    # since we know that there are 6012 pneumonia cases and 20672 non-pneumonia cases we will balance the 2 datasets 
    x_train = np.zeros((train_image_count,img_width,img_height,3), dtype=np.float32)
    y_train = np.zeros((train_image_count,2), dtype=np.float32)
    train_class_0_count = 0
    train_class_1_count = 0
    total_train = 0
    train_images_to_process = train_image_count
    train_images_0_to_process = train_images_to_process/2
    train_images_1_to_process = train_images_to_process/2
    
    for key in data:
        img = load_image(data[key]['image_path'],img_width,img_height)
        label = data[key]['target']
        
        #check if label is 0 and we haven't reached the max count else skip
        if label == 0 and train_class_0_count < train_images_0_to_process:
            x_train[train_class_0_count]= img
            y_train[train_class_0_count]= [0,1]
            train_class_0_count += 1
        #check if label is 1 and we haven't reached the max count else skip
        elif label == 1 and train_class_1_count < train_images_1_to_process:
            x_train[train_class_1_count]= img
            y_train[train_class_1_count]= [1,0]
            train_class_1_count += 1 
        
        # exit strategy
        if (train_class_0_count + train_class_1_count >= train_images_to_process):
            print('training data class 0 processed==>'+str(train_class_0_count))
            print('training data class 1 processed==>'+str(train_class_1_count))
            break   
...
def prepare_data(data,img_width,img_height):
    ...
    print('training labels processed==>'+str(y_train.shape))
    return X_train, y_train
...
#         X, y = prepare_data(metainfo,128,128)
        X, y = data_downsample(metainfo,10,128,128) #passing 12024 so that we get 6012 images of each class
